...
public class MarcadorBaloncesto {
     ...
     * 
     * @exception IllegalArgumentException si el equipo no es 'L' ni 'V'
     * @exception IllegalArgumentException si los puntos no son 1, 2 o 3
     */
    public void añadirCanasta(char equipo, int puntos) throws IllegalArgumentException {
        if (!((puntos>0)&(puntos<4))) throw new IllegalArgumentException("Número de puntos por canasta no válidos (distinto de 1, 2 o 3)");
        ...
        switch(equipo){
            ...
            case 'V':
                ...
                break;
            default:
                throw new IllegalArgumentException("Equipo no válido (distinto de 'L' o 'V')");
        }        
    }
     ...
     * 
     * @exception IOException si la ruta no es un archivo, o si no se puede escribir
     */
    ...
    /**
     * Lee los datos del archivo de texto cuya ruta se pasa como parámetro y los devuelve en un String
     * @param ruta el archivo a leer
     * @return un String con nombre y puntos del equipo local y nombre y puntos del equipo visitante
     * 
     * @exception IOException si la ruta no es un archivo, o si no se puede leer
     */
    public String cargar(String ruta) throws IOException {
        File archivo = new File(ruta);
        if (!archivo.exists()) new IOException("No se encontró el archivo");        
        if (!archivo.isFile()) new IOException("No es un archivo");
        if (!archivo.canRead()) new IOException("No hay permisos para leer");   
